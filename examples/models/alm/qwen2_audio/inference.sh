...
export MEGATRON_PATH="/workspace_yuekai/asr/Megatron-Bridge/examples/models/alm/qwen2_audio/qwen2_audio_7b_instruct_2512_bf16"
...
# uv run python -m torch.distributed.run --nproc_per_node=2 examples/conversion/hf_to_megatron_generate_alm.py \
...
uv run python -m torch.distributed.run --nproc_per_node=1 examples/conversion/hf_to_megatron_generate_alm.py \
  --hf_model_path ${HF_MODEL} \
