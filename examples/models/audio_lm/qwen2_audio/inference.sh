...
# export PYTHONPATH=/workspace_yuekai/asr/Qwen3-ASR:/workspace_yuekai/asr/Megatron-qwen3-asr/src:$PYTHONPATH
export PYTHONPATH=/workspace_yuekai/asr/Megatron-qwen3-asr/src:$PYTHONPATH
export WORKSPACE=${WORKSPACE:-/workspace}
# export HF_MODEL="/workspace_yuekai/HF/Qwen2-Audio-7B-Instruct"
export HF_MODEL="/workspace_yuekai/HF/Qwen3-ASR-1.7B"
...
/workspace_yuekai/asr/Megatron-Bridge/.venv/bin/python -m torch.distributed.run --nproc_per_node=2 examples/conversion/hf_to_megatron_generate_audio_lm.py \
  --hf_model_path ${HF_MODEL} \
  --audio_url "${AUDIO_URL_3}" \
  --prompt "" \
  --tp 2 \
  --max_new_tokens 50
