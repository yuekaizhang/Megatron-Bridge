...
def load_audio(audio_path: str, sampling_rate: int = 16000):
    ...
    if not HAS_LIBROSA:
        raise ImportError(
            "librosa is required for audio loading. " "Please install it: pip install librosa"
        )
...
def main(args) -> None:
    ...
    prompt = args.prompt
    input_ids, input_features, feature_attention_mask, messages = process_audio_inputs(
        processor, audio_path, prompt
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Audio-Language Generation from HuggingFace Audio-Language Models"
    )
    parser.add_argument(
        "--hf_model_path",
        type=str,
    ...
    parser.add_argument(
        "--megatron_model_path", type=str, default=None, help="Path to the Megatron model checkpoint"
    )
    parser.add_argument(
        "--audio_path",
        type=str,
